...
Allergens are matched a little more carefully than a plain word search. A category such as dairy is expanded to the ingredient words recipes actually use — butter, cream, cheese, ghee, paneer — because almost no recipe lists the word "dairy" itself. A custom allergy falls back to matching the word as typed. As with diet, matching is word-boundary aware, so egg never trips on eggplant.
